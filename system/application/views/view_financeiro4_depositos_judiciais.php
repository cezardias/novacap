...
function excluir(IdPgtoAcao,AcaoId){
	...
	if (confirma){
		window.location = "<?php echo base_url();?>cfinanceiro/delete/"+IdPgtoAcao+"/"+AcaoId+"/4"; 
	} else {
		return false;
	}
}		
...
<div class="row" style="width:960px;">
	<div class="well well-sm" style="width:960px;">
		<h2>Financeiro - Depósitos judiciais</h2>
		<font size="3">
...
<form role="form" method="post" action="<?php echo base_url();?>cfinanceiro/createdepositojud" name="formdepjud">
<input type="hidden" name="acaoid" id="acaoid" value="<?php echo $AcaoId?>">

<div class="row col-xs-12" style="width:960px;margin-left:-30px;">
	<div class="col-xs-3">
		<label class="control-label" style="margin-top:8px;margin-bottom:-2px;">Data da inscrição</label>
...
<?php }
if(!empty($depositosjud)&&(sizeof($depositosjud)>0)){?>
<div class="row">
	<div class="col-md-6">
		<table class="table" style="width:960px;margin-left:-15px;">
			...
			<tbody>
				<?php foreach ($depositosjud as $pgtoac):
