...
function excluir(IdPgtoAcao,AcaoId){
	...
	if (confirma){
		window.location = "<?php echo base_url();?>cfinanceiro/delete/"+IdPgtoAcao+"/"+AcaoId+"/8"; 
	} else {
		return false;
	}
}		
...
<div class="row" style="width:960px;">
	<div class="well well-sm" style="width:960px;">
		<h2>Financeiro - Bloqueios judiciais</h2>
		<font size="3">
...
<form role="form" method="post" action="<?php echo base_url();?>cfinanceiro/createbloqueiojud" name="formbloqjud">
<input type="hidden" name="acaoid" id="acaoid" value="<?php echo $AcaoId?>">

<div class="row col-xs-12" style="width:960px;margin-left:-30px;">
	<div class="col-xs-3">
		<label class="control-label" style="margin-top:8px;margin-bottom:-2px;">Data de inscrição</label>
	...
	<div class="col-xs-3 selectContainer">
		...
		<select class="form-control" name="contabanco" required>
			<option value=""></option>
			<?php foreach ($contasbanco as $conts):
				$IdConta = $conts->Id;
				$Banco = $conts->Banco;
				$Agencia = $conts->Agencia;
				$Conta = $conts->Conta;
				$ContaCompleta = $conts->Combobox;
				$Complemento = $conts->Complemento;?>
				<option value="<?php echo $IdConta?>"><?php echo $ContaCompleta?></option>
			<?php endforeach;?>
		</select>
	</div>	
	<div class="col-xs-3">
		<label class="control-label" style="margin-top:8px;margin-bottom:-2px;">Valor</label>
		<input type="text" class="form-control" name="valorpgtoacao" id="valorpgtoacao" value="" style="text-align:right;" onKeyPress="return MascaraMoeda(this,'.',',',event);" style="text-align:right;" required>					
	</div>
	<div class="col-xs-12">
...
<?php }
if(!empty($bloqjudiciais)&&(sizeof($bloqjudiciais)>0)){?>
<div class="row">
	<div class="col-md-6">
		<table class="table" style="width:960px;margin-left:-15px;">
			<thead>
				<tr style="background-color:#D3D3D3">
					...
					<th style="width:200px;text-align:center;">CONTA</th>
					<th style="width:500px;text-align:left;">OBSERVAÇÕES</th>
					<th style="width:30px;text-align:left;"></th>
				</tr>
			</thead>
			<tbody>
				<?php foreach ($bloqjudiciais as $itens):
				$IdPgtoAcao = $itens->Id;
				$Data = $itens->Data; if($Data==""){$Data='';}else{$Data = date('d/m/Y',strtotime($Data));}
				...
				<tr>
					...
					<td style="width:200px;text-align:right;"><?php echo $Conta?></td>
					<td style="width:500px;text-align:left;text-transform:uppercase;"><?php echo utf8_encode($Observacao)?></td>
					<td style="Widht:30">
					<?php if(($_SESSION['Admin']==1)||($_SESSION['Nivel5']==1)){ // 5 FINANCEIRO - ADMINISTRADOR?>
