...
foreach ($detailautuacao as $item):
	$Sisprot = $item->Sisprot; if((empty($Sisprot)||($Sisprot=="NULL"))){$Sisprot="-";}
	$Autor = $item->Autor; if((empty($Autor)||($Autor=="NULL"))){$Autor="-";}
	$Reu = $item->Reu; if((empty($Reu)||($Reu=="NULL"))){$Reu="-";}
	$Assunto = $item->Assunto; if((empty($Assunto)||($Assunto=="NULL"))){$Assunto="-";}else{$Assunto=substr($Assunto,0,46);}
	$ProcessoJudicialNumero = $item->ProcessoJudicialNumero; if((empty($ProcessoJudicialNumero)||($ProcessoJudicialNumero=="NULL"))){$ProcessoJudicialNumero="-";}
	$ProcessoJudicialNumeroAntigo = $item->ProcessoJudicialNumeroAntigo; if((empty($ProcessoJudicialNumeroAntigo)||($ProcessoJudicialNumeroAntigo=="NULL"))){$ProcessoJudicialNumeroAntigo="-";}
	$Vara = $item->Vara; if((empty($Vara)||($Vara=="NULL"))){$Vara="-";}
	$AudienciaDataHora = $item->AudienciaDataHora; if($AudienciaDataHora==""){$AudienciaDataHora='';}else{$AudienciaDataHora = date('d/m/Y H:i',strtotime($AudienciaDataHora));}
	$AudienciaTipo = $item->AudienciaTipo; if((empty($AudienciaTipo)||($AudienciaTipo=="NULL"))){$AudienciaTipo="";}
	$Data = $item->Data; if((empty($Data)||($Data=="NULL"))){$Data="";}	
endforeach;
...
$col[] = array('text' => ': '.$ProcessoJudicialNumeroAntigo, 'width' => '160', 'height' => '8', 'align' => 'L', 'font_name' => 'Arial', 'font_size' => '12', 'font_style' => '', 'fillcolor' => '255,255,255', 'textcolor' => '0,0,0', 'drawcolor' => '0,0,0', 'linewidth' => '0.1', 'linearea' => 'R');	
$titulo[] = $col;
$pdf->WriteTable($titulo);

$titulo = array();
$col = array();	 
$col[] = array('text' => 'CNJ', 'width' => '30', 'height' => '8', 'align' => 'L', 'font_name' => 'Arial', 'font_size' => '13', 'font_style' => '', 'fillcolor' => '255,255,255', 'textcolor' => '0,0,0', 'drawcolor' => '0,0,0', 'linewidth' => '0.1', 'linearea' => 'L');
...
$titulo = array();
$col = array();	 
$col[] = array('text' => 'Brasília, '.$dia.' de '.$mes.' de '.$ano, 'width' => '190', 'height' => '8', 'align' => 'C', 'font_name' => 'Arial', 'font_size' => '13', 'font_style' => '', 'fillcolor' => '255,255,255', 'textcolor' => '0,0,0', 'drawcolor' => '0,0,0', 'linewidth' => '0.1', 'linearea' => '');	
$titulo[] = $col;
$pdf->WriteTable($titulo);
$titulo = array();
$pdf->Ln(42);
