...
function excluir(IdPgtoAcao,AcaoId){
	...
	if (confirma){
		window.location = "<?php echo base_url();?>cfinanceiro/delete/"+IdPgtoAcao+"/"+AcaoId+"/9"; 
	} else {
		return false;
	}
}		
...
<div class="row" style="width:960px;">
	<div class="well well-sm" style="width:960px;">
		<h2>Financeiro - Bloqueios judiciais - CONVOLADOS</h2>
		<font size="3">
...
<form role="form" method="post" action="<?php echo base_url();?>cfinanceiro/createbloqueiosjudconvolados" name="formdepjud">
<input type="hidden" name="acaoid" id="acaoid" value="<?php echo $AcaoId?>">

<div class="row col-xs-12" style="width:960px;margin-left:-30px;">
	<div class="col-xs-3">
		<label class="control-label" style="margin-top:8px;margin-bottom:-2px;">Data de baixa</label>
...
<?php }
if(!empty($bloqueiosjudconvolados)&&(sizeof($bloqueiosjudconvolados)>0)){?>
<div class="row">
	<div class="col-md-6">
		<table class="table" style="width:960px;margin-left:-15px;">
			...
			<tbody>
				<?php foreach ($bloqueiosjudconvolados as $itens):
				$IdPgtoAcao = $itens->Id;
