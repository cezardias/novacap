...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', 'DepositoJudicial');
...
foreach ($resultvlc as $it):
  ...
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$linha, $ProbabilidadeDePerda);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$linha, $DepositoJudicial);
...
$hora = date('H');
$min = date('i');
$nome = 'Depositos_Jud_'.$data.'-'.$hora.$min;
...
<?php $this->load->view('view_cabecalho');?>
	<h2>Financeiro Depósito Judicial</h2>
