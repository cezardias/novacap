...
class Cjuridico extends Controller {
	...
	function buscaprazoindex(){
		...
		foreach ($data['advogadonivel'] as $advn):
			$AdvgNivel = $advn->Nivel; //Advogado chefe tem acesso todal
	...
	function buscacaotrabresult() {
	  ...
	  $config['next_link'] = 'Próximo';
		...
		$totalLinhas = $this->mjuridico->getAcoesQtd($interesado,$prjudnum,$pradmnum,$assuntoid,$advogadoid,$varaid,$obsbusca,$statusprocessoid,$andamentoid,$posicaonovacap,$cpfcnpj,$dtajuizamentoini,$dtajuizamentofim,$dtextincaoini,$dtextincaofim,$prsei);
		$totalLinhas2 = 0;
		...
		if($query = $this->mjuridico->getAcoesResult($interesado,$prjudnum,$pradmnum,$assuntoid,$advogadoid,$varaid,$obsbusca,$statusprocessoid,$andamentoid,$posicaonovacap,$cpfcnpj,$dtajuizamentoini,$dtajuizamentofim,$dtextincaoini,$dtextincaofim,$prsei,$per_page,$this->uri->segment(3))){
	...
	function geraexcel(){
		...
		$dtextincaofim = $this->session->userdata('dtextincaofim');
		$prsei = $this->session->userdata('prsei');
		$data['docresult'] = $this->mjuridico->getAcoesExcel($interesado,$prjudnum,$pradmnum,$assuntoid,$advogadoid,$varaid,$obsbusca,$statusprocessoid,$andamentoid,$posicaonovacap,$cpfcnpj,$dtajuizamentoini,$dtajuizamentofim,$dtextincaoini,$dtextincaofim,$prsei);
	...
	function createcontrato(){
		...
		$contratoobjeto = $this->input->post('contratoobjeto'); if($contratoobjeto==""){$contratoobjeto=NULL;}
		$observacoes = $this->input->post('observacoes'); if($contratoobjeto==""){$contratoobjeto=NULL;}
		$data = array(
			'ContratoNr' => $contratonr,
	...
	function savecontrato(){
		...
		$prazodevigencia = $this->input->post('prazodevigencia'); if($prazodevigencia==""){$prazodevigencia=NULL;}
		$prazodevigenciatipo = $this->input->post('prazodevigenciatipo'); if($prazodevigenciatipo==""){$prazodevigenciatipo=NULL;}
		$contratoobjeto = $this->input->post('contratoobjeto');
