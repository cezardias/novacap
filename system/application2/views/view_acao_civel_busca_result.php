...
if(!empty($docresult)&&(sizeof($docresult)>0)){
	$mostrabtn = 'exibe';
	if($total_linhas == 1){
		$total_linhas_mensagem = ' 1 a&ccedil;&atilde;o c&iacute;vel encontrada.';
	}
	if($total_linhas > 1){
		$total_linhas_mensagem = ' '.number_format($total_linhas, 0, ',', '.').' a&ccedil;&otilde;es c&iacute;veis encontradas.';
	}?>
	...
	<table class="data_grid" cellspacing="0">
		<thead>
			<tr>
				...
				<td width=17% class="middle" style="text-align:center;"><b>R&eacute;u</b></td>
				<td width=13% class="middle" style="text-align:left;"><b>Advogado</b></td>
			</tr>
		</thead>
		<tfoot>
		</tfoot>
		...
		foreach($docresult as $its):
			...
			<tr style="border:0;">
				<td class="middle" style="text-align:left;">
					<a href="<?php echo base_url();?>cacaocivel/detailacaocivel/<?php echo $IdAcao;?>" title="Detalhar solicita&ccedil;&atilde;o" style="color:blue;font-size:11px;">
						<?php echo $ProcessoJud?>
					</a>
				</td>
				<td class="middle" style="text-align:center;text-transform:uppercase;font-size:11px;"><?php echo $ProcessoAdmin?></td>
				<td class="middle" style="text-align:center;text-transform:uppercase;font-size:11px;"><?php echo $PrJudNumAntigo?></td>

				<?php if($Autor != "-"){ // facilitar o - centralizado quando for vazio?>
					<td class="middle" style="text-align:left;text-transform:uppercase;font-size:11px;"><?php echo $Autor?></td>
				<?php }else{?>
					<td class="middle" style="text-align:center;text-transform:uppercase;font-size:11px;"><?php echo $Autor?></td>
				<?php }?>

				<?php if($Reu != "-"){ // facilitar o - centralizado ?>
					<td class="middle" style="text-align:left;text-transform:uppercase;font-size:11px;"><?php echo $Reu?></td>
				<?php }else{?>
					<td class="middle" style="text-align:center;text-transform:uppercase;font-size:11px;"><?php echo $Reu?></td>
				<?php }?>

				<td class="middle" style="text-align:left;font-size:11px;"><?php echo $AcaoAdvogado?></td>
			</tr>
	<?php endforeach;?>
		</tbody>
	</table>
